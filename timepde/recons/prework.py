...
class Trainer:
    def __init__(self,rank,worker_num,
        ...
        self.recv_comm_weight = [[] for i in range(worker_num)]
        for u,edge_list in enumerate(self.send_graph):
            for i,v in enumerate(edge_list):
                self.recv_graph[v].append(u)
                if self.comm_weight == None:
                    self.recv_comm_weight[v].append(1)
                else:
                    self.recv_comm_weight[v].append(self.comm_weight[u][i])
                    
        if print_flag == True:
            if rank == 0:
                print_ranked('t_span_list',self.t_span_list)
                print_ranked('send_t_span_list',self.send_t_span_list)
                print_ranked('send_graph',self.send_graph)
                print_ranked('recv_graph',self.recv_graph)
                print_ranked('recv_comm_weight',self.recv_comm_weight)

        
    def assign_task(self, cloud_point_list,loss_list, output_dim):
        ...
        if self.rank == 0:
            ...
            for u,edge_list in enumerate(self.send_graph):
                for i,v in enumerate(edge_list):
                    ...
                    recv_ydata_shape_item[-1] = output_dim
        ...
        self.local_recv_comm_weight = scatter_objects(
            self.recv_comm_weight if self.rank == 0 else None)
    ...
    def train(self, epoch_num, info_exchange_round, test_round, result_dir,
        ...
        loss_filter = \
                    ...
                    if include]
        final_weight = [1] * len(loss_filter) + self.local_recv_comm_weight
        print_ranked("weight", final_weight)
